...
function loadSettings() {
    ...
    const autoStartSwitch = document.querySelector('.switch');
    autoStartSwitch.setAttribute('checked', autoStart);
    
    const longBreakInterval = localStorage.getItem('longBreakInterval') || '4';
    const intervalInput = document.querySelector('.input-lb');
    intervalInput.value = longBreakInterval;
}

function saveSettings() {
    const autoStart = document.querySelector('.switch').getAttribute('checked') === 'true';
    const longBreakInterval = document.querySelector('.input-lb').value;
...
function toggleAutoStart() {
    const autoStartSwitch = document.querySelector('.switch');
    const currentState = autoStartSwitch.getAttribute('checked');
    const newState = currentState === 'true' ? 'false' : 'true';
    autoStartSwitch.setAttribute('checked', newState);
    
    saveSettings();
    
    const status = newState === 'true' ? 'ENABLED' : 'DISABLED';
...
function startCountdownForCurrentTimer() {
    ...
    countdown = setInterval(function() {
        if (sec == "00") {
            if (min == "00") {
                ...
                if (document.querySelector(".switch").getAttribute("checked") == "true") {
                    setTimeout(() => {
                        document.querySelector(".start-button").innerHTML = "PAUSE";
                        skip_button();
                        startCountdownForCurrentTimer();
                    }, 100);
                }
                return;
            }
...
document.querySelector(".input-lb").addEventListener("change", function() {
    saveSettings();
});
